...
// 리턴받은 값
var resultPw = '<%=request.getAttribute("pw")%>';
...
window.onload = function pw(){
	console.log(resultPw);

		
		if(resultPw == ''){
			document.getElementById('resultPw').value = '조건에 맞는 비밀번호가 존재하지 않습니다.';
		}
		if(resultPw.length > 4){
			document.getElementById('resultPw').value = '비밀번호는 < ' + '<%=request.getAttribute("pw")%>' + ' >입니다.';
		}
...
회원가입 시 입력하신 아이디, 이메일, 생년월일로  비밀번호를 찾으실 수 있습니다.
<br>
	
	<form action="pwFind.five" method="post" onsubmit="return check()">
	<table>
		<tr>
		<th>아이디 : </th>
		<td><input type="text" placeholder="YOUR ID" name="inputId" id="inputId"  ></td>
		</tr>
		<tr>
			<th>이메일 :</th>
		...
		<tr>
			<th>생년월일 :</th>
			 <td><input type="text" name="inputBirth" placeholder="YOUR BIRTH" id="inputBirth" size="10" maxlength="15" readonly="readonly">&nbsp;
</td>
		</tr>
		<tr>
		<td colspan="3">
				<div id="home3">
				<input type="submit" style="width: 150px; height: 36px; background-color: white;" name="find" id="find" value="비밀번호 찾기"></div>
					</td>
		</tr>
		<tr>
			<th>비밀 번호 :</th>
			<td> <input type="text" name="resultPw" id="resultPw" placeholder="PASSWORD" size="45"  readonly="readonly">
